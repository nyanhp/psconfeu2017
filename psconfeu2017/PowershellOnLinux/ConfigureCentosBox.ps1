        ...
        foreach ($Computer in $ComputerName)
        {
            ...
            foreach ($result in $results)
            {
                ...
                if ($item[1] -notmatch '^\[::')
                {
                    ...
                    $procId = $item[-1]
                    $proto = $item[0]
                    $status = if ($item[0] -eq 'tcp') {$item[3]}
                    else {$null}	
...
#region Initial system configuration
...
Invoke-SSHCommand -SSHSession $Session -Command "yum install -y powershell Glibc python openssl python-ctypes libcurl unzip wget"

Invoke-SSHCOmmand -SSHSession $Session -Command "yum downgrade openssl" #Or modify symbolic links in /opt/omi/lib

Invoke-SSHCommand -SSHSession $Session -Command "wget https://github.com/Microsoft/omi/releases/download/v1.2.0-35/omi-1.2.0-35.ssl_098.x64.rpm"

Invoke-SSHCommand -SSHSession $Session -Command "wget https://github.com/Microsoft/PowerShell-DSC-for-Linux/releases/download/v1.1.1-294/dsc-1.1.1-294.ssl_100.x64.rpm"

Invoke-SSHCommand -SSHSession $Session -Command "rpm -Uvh omi-1.2.0-35.ssl_098.x64.rpm dsc-1.1.1-294.ssl_100.x64.rpm"

Invoke-SSHCommand -SSHSession $Session -Command "yum install omi-psrp-server"

Invoke-SSHCommand -SSHSession $Session -Command "firewall-cmd --add-port 5985/tcp --permanent; firewall-cmd --add-port 5986/tcp --permanent"

Set-Item WSMan:\localhost\Client\AllowUnencrypted $true -Force -Confirm:$false
Set-Item WSMan:\localhost\Client\TrustedHosts * -Force -Confirm:$false
#endregion
...
Get-NetworkStatistics -ShowHostnames -Port 5985
$Sessions = Remove-PSSession

$sshSession = New-PSSession -SSHTransport -HostName Centos1 -UserName root
...
Get-NetworkStatistics -ShowHostnames -Port 22
#endregion
...
configuration PullClientOsAgnostic
{
    param
    (
        [string[]]
    ...
    node $ComputerName
    {
        ...
        ConfigurationRepositoryWeb xpull
        {
